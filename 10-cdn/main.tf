resource "aws_cloudfront_distribution" "learnaws" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.learnaws.domain_name
        zone_id = aws_cloudfront_distribution.learnaws.hosted_zone_id
      }
    }
  ]
}